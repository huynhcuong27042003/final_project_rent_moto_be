...
class BookingMotoService{
    ...
    async deleteBooking({ email, numberPlate }) {
        try {
            let querySnapshot;

            if (email) {
                // Truy vấn các booking có email tương ứng
                querySnapshot = await db.collection('bookings').where('email', '==', email).get();
                if (querySnapshot.empty) {
                    throw new Error(`No bookings found for email ${email}.`);
                }
            } else if (numberPlate) {
                // Truy vấn các booking có numberPlate tương ứng
                querySnapshot = await db.collection('bookings').where('numberPlate', '==', numberPlate).get();
                if (querySnapshot.empty) {
                    throw new Error(`No bookings found for number plate ${numberPlate}.`);
                }
            } else {
                throw new Error('You must provide either email or numberPlate.');
            }

            // Xóa tất cả các booking tìm thấy
            const deletePromises = querySnapshot.docs.map(doc => doc.ref.delete());
            await Promise.all(deletePromises);

            if (email) {
                console.log(`All bookings for email ${email} have been deleted.`);
                return { message: `All bookings for email ${email} have been deleted.` };
            }

            if (numberPlate) {
                console.log(`All bookings for number plate ${numberPlate} have been deleted.`);
                return { message: `All bookings for number plate ${numberPlate} have been deleted.` };
            }
        } catch (error) {
            console.error('Error deleting bookings:', error);
            throw new Error('Could not delete bookings from Firestore');
        }
    }
