...
class ReviewService {
    ...
    // Add a new review
    async addReview(email, numberPlate, image, numberStars, comment) {
        try {
            ...
            const newReview = new Review(email, numberPlate, image, numberStars, comment);
    ...
    async updateReview(id, updates) {
        try {
            ...
            // Handle nested fields, if any (e.g., image, comment)
            if (updates.image === null || updates.image === undefined) {
                delete updatedData.image; // Optional: If no image, remove it from update
            }
