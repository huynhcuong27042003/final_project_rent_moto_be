...
app.post('/api/momo/callback', (req, res) => {
  const { partnerCode, orderId, requestId, amount, resultCode, message, extraData } = req.body;
  ...
  if (resultCode === 0) {
      ...
      console.error(`Payment failed for Order ID: ${orderId} with message: ${message}`);
      return res.status(400).json({ message: 'Payment failed', orderId, error: message });
  }
});
