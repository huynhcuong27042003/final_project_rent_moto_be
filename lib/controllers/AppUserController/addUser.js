...
exports.addUser = async (req, res) => {
    try {
        ...
        const { email, phoneNumber, role = "user", address, information, isActive = true, isHide = false} = req.body; // Default role to "user"

        // Validate input data
        if (!email || !phoneNumber || !role || typeof address !== 'object') {
        ...
        const newUser = await AppUserService.addUser(
            ...
            role,
            address,
            infoInstance, // Using the Information instance here
            isActive,
            isHide
        );
